...
**Flow:** `main.py` → (for each language) fresh browser → `browser.initialize_session` (home → accept terms → dashboard → select language → new test) → `pagination.scrape_test` (async generator, yields one question at a time) → `extractor.extract_question` (per-question extraction) → `output.write_progress` (rewrites JSON after each question).
...
|---|---|---|
| Question / option text | `div.Qe5J54-r` (visible, `innerText`) | `span.vK65g4nl` (CSS-hidden, `textContent`) |
